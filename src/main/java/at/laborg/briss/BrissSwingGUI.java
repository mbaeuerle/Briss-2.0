...
public class BrissSwingGUI implements BrissGUIApp {
        ...
        if (fileArg.exists() && fileArg.getAbsolutePath().trim().endsWith(".pdf")) { // $NON-NLS-1$
            try {
                ...
                JOptionPane.showMessageDialog(
                        mainWindow,
                        e.getMessage(),
                        Messages.getString("BrissGUI.brissError"), // $NON-NLS-1$
    ...
    private void loadPDF(File file) {
        try {
            ...
            JOptionPane.showMessageDialog(
                    mainWindow,
                    e.getMessage(),
                    Messages.getString("BrissGUI."), // $NON-NLS-1$
                    JOptionPane.ERROR_MESSAGE);
        } catch (PdfException e) {
            JOptionPane.showMessageDialog(
                    mainWindow,
                    e.getMessage(),
                    Messages.getString("BrissGUI.loadingError"), // $NON-NLS-1$
                    JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void savePDF(File file) {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(
                    mainWindow,
                    e.getMessage(),
                    Messages.getString("BrissGUI.croppingError"), // $NON-NLS-1$
                    JOptionPane.ERROR_MESSAGE);
        } finally {
    ...
    private void setStateAfterClusteringFinished(
            ClusterDefinition newClusters, PageExcludes newPageExcludes, File newSource, String password) {

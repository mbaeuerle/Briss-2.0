...
public final class BrissFileHandling {
    ...
    public static boolean checkValidStateAndCreate(File destinationFile) throws IOException, IllegalArgumentException {
        if (destinationFile == null) throw new IllegalArgumentException("Destination File musst be set!");
        if (!destinationFile.exists()) return destinationFile.createNewFile();
        return true;
    }
}

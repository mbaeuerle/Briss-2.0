...
public class MergedPanel extends JPanel {
    ...
    private enum ActionState {
        ...
        MOVE_CROP,
        RESIZING_HOTCORNER_UR,
        RESIZING_HOTCORNER_LL,
        RESIZING_RIGHT_EDGE,
        RESIZING_LEFT_EDGE,
        RESIZING_UPPER_EDGE,
        RESIZING_LOWER_EDGE
    ...
    private static final List<Pair<BiPredicate<DrawableCropRect, Point>, Integer>> CURSORS_FROM_CROP_AND_POINT =
            new ArrayList<>(9);
    ...
    public void resizeSelCrop(int width, int height) {
        for (DrawableCropRect crop : crops) {
            if (crop.isSelected()) {
                if (((width < 0) && (crop.width <= -width)) || ((height < 0) && (crop.height <= -height))) return;
    ...
    public void selectCrops(boolean select) {
        for (DrawableCropRect crop : crops) crop.setSelected(select);
        repaint();
    }

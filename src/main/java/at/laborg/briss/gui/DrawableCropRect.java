...
public class DrawableCropRect extends Rectangle {
    ...
    private Font scaleFont(String text, FontMetrics fontMetrics) {
        ...
        if (width == 0 || height == 0) return MergedPanel.BASE_FONT;
        ...
        return super.contains(p)
                || isSelected()
                        && (isOverRightEdge(p)
                                ...
                                || isOverUpperEdge(p)
                                || isOverLowerEdge(p)
                                || cornerUpperLeft.contains(p)
                                || cornerUpperRight.contains(p)
                                || cornerLowerLeft.contains(p)
                                || cornerLowerRight.contains(p));
    }
    ...
    static class ResizeHandle {
        ...
        public void draw(Graphics2D g2) {
            ...
            g2.fillRect(
                    corner.x - halfCornerDimension, corner.y - halfCornerDimension, CORNER_DIMENSION, CORNER_DIMENSION);
            g2.setColor(Color.BLACK);
            g2.drawRect(
                    corner.x - halfCornerDimension, corner.y - halfCornerDimension, CORNER_DIMENSION, CORNER_DIMENSION);
        }

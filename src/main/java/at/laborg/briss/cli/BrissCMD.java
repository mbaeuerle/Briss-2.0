...
public final class BrissCMD {
    ...
    public static void autoCrop(final String[] args) {
        ...
        if (!CommandValues.isValidJob(workDescription)) return;
        ...
        try {
            for (PageCluster cluster : clusterDefinition.getClusterList()) {
                float[] auto =
                        CropFinder.getAutoCropFloats(cluster.getImageData().getPreviewImage());
                ...
                if (workDescription.isSplitColumns()) {
                    ...
                    for (float[] crop : ratios) {
                        newRatios.addAll(
                                SplitFinder.splitColumn(cluster.getImageData().getPreviewImage(), crop));
                    }
                    ratios = newRatios;
                }
                ...
                if (workDescription.isSplitRows()) {
                    ...
                    for (float[] crop : ratios) {
                        newRatios.addAll(
                                SplitFinder.splitRow(cluster.getImageData().getPreviewImage(), crop));
                    }
                    ratios = newRatios;
                }
            ...
            CropDefinition cropDefintion = CropDefinition.createCropDefinition(
                    workDescription.getSourceFile(), workDescription.getDestFile(), clusterDefinition);

...
public final class CropDefinition {
    ...
    private CropDefinition(
            final File source, final File destination, final HashMap<Integer, List<float[]>> pageToCropRectangles) {
        this.sourceFile = source;
        this.destinationFile = destination;
        this.pageToCropRectangles = pageToCropRectangles;
    }

    public static CropDefinition createCropDefinition(
            final File source, final File destination, final ClusterDefinition clusters) throws IOException {
        if (source == null) throw new IllegalArgumentException("Source must be provided");
        if (!source.exists())
    ...
    public List<float[]> getRectanglesForPage(final Integer page) {
        if (pageToCropRectangles.containsKey(page)) return pageToCropRectangles.get(page);
